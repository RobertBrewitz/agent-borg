...
Recovery: hive.sh auto-recovers stale in-progress/ plans (no active agent for
STALE_TIMEOUT seconds) back to todo/, up to MAX_RETRIES times. After that,
plans move to blocked/. Use --retry-blocked to re-queue all blocked plans.
```
...
- **in-progress/** — Plan is actively being worked on (a worktree exists for it). Hive recovers stale plans here (no active agent for `STALE_TIMEOUT`) back to `todo/`.
...
- **merge/** — Reserved for merge workspace. Not polled by hive.sh.
- **archive/** — Merged to main. Historical record of completed and integrated work.
- **blocked/** — A step failed after 3 attempts, or hive exhausted `MAX_RETRIES` re-dispatches. Use `hive.sh --retry-blocked` to move all back to `todo/`.
